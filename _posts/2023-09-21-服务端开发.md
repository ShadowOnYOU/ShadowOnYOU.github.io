...
1. 方法参数注入（Method Parameter Injection）：这种方式通过在方法参数上使用 `@Autowired` 注解来实现注入。例如：
...
自动化配置相关的注解：component，autowired，componentscan，configuration
...
混合配置：javaconfig：import（）导入配置类，importsource（）导入xml文件
